        ...
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <form class="float-end" action="{{route('admin.orders.index')}}">
                    ...
                    <input type="search" name="q" id="" value="{{ $search }}" placeholder="Nhập mã đơn hàng hoặc số điện thoại">
                        ...
                        <table id="example" class="table table-striped" style="width:100%">
                            ...
                            @foreach($orders as $order)
                                <tr>
                                    ...
                                    <td>{{$order->name}}</td>
                                    <td>{{$order->phone}}</td>
                                    <td>{{$order->order_dateDMYHM}}</td>
                                    <td>{{$order->total_priceVND}}</td>
                                    <td>
                                        @if($order->status === 0)
                                            <span class="badge bg-yellow">{{$order->status_name}}</span>
                                        @elseif($order->status === 1)
                                            <span class="badge bg-blue">{{$order->status_name}}</span>
                                        @elseif($order->status === 2)
                                            <span class="badge bg-green">{{$order->status_name}}</span>
                                        @elseif($order->status === 3 || $order->status === 4)
                                            <span class="badge bg-red">{{$order->status_name}}</span>
                                        @endif
                                    </td>
